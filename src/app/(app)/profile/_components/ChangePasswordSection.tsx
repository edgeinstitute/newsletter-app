...
export function ChangePasswordSection({ email }: Props) {
  ...
  return (
    <section className="rounded-xs border border-border bg-surface-elevated p-6">
      <h2 className="text-lg font-medium text-foreground">Schimbă parola</h2>
      <p className="mt-1 text-sm text-text-muted">
        Introdu parola actuală și alege una nouă.
      </p>

      <form onSubmit={onSubmit} className="mt-5 flex flex-col gap-4" noValidate>
        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium text-text-secondary">Parola actuală</span>
          <input
            type="password"
        ...
        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium text-text-secondary">Parolă nouă</span>
          <input
            type="password"
          ...
          {next && <p className="text-xs text-text-muted">Siguranță: {strength.label}</p>}
        </label>

        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium text-text-secondary">Confirmă parola nouă</span>
          <input
            type="password"
            autoComplete="new-password"
            required
